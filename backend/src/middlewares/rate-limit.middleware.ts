...
const globalRateLimiter = rateLimit({
  ...
  handler: (request: Request, response: Response, next: NextFunction, options: any): Response => {
    logger.warn(
    ...
    return response.status(429).json({
      ...
      retryAfter: (request as any).rateLimit?.resetTime
    });
  }
});
...
const financialProfileRateLimiter = rateLimit({
  ...
  handler: (request: Request, response: Response, next: NextFunction, options: any): Response => {
    logger.warn(
      { ip: request.ip, path: request.path, userId: (request as any).user?.id },
